            ...
            <div class="row search-page-top d-flex align-items-center justify-content-center">
                <div class="banner-content col-lg-12">
                    ...
                    <p class="text-white link-nav">
                        <a href="/">Home </a> <span class="lnr lnr-arrow-right"></span> <a href="/search">
                            Job details page</a>
                    </p>
                    ...
                    @if(isset($category_search_custom))
                        <p class="text-white">{{count($category_search_custom)}} Results found for <span> {{Request::input('search')}} , {{Request::input('select_area_search')}} , {{Request::input('select_category_search')}}</span></p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="container">
            <div class="row justify-content-center d-flex">
                <div class="col-lg-8 post-list">
                        ...
                        @guest
                            @if(isset($category_search_custom))
                                @if((count($category_search_custom)) > 0)
                                    @foreach($category_search_custom as $category_search_cus)
                                            ...
                                            <div class="details">
                                                <div class="title d-flex flex-row justify-content-between">
                                                    <div class="col-md-11 col-sm-11">
                                                        <div class="titles">
                                                            <a href="/details/{{$category_search_cus->id_posts}}">
                                                                <h4>{{$category_search_cus->job_title}}</h4></a>
                                                            <h6>{{$category_search_cus->company_name}}</h6>
                                                        </div>
                                                    </div>
                                                ...
                                                <p>
                                                    {{$category_search_cus->description_work}}
                                                </p>
                                                <h5>Job Nature: {{$category_search_cus->name_type_work}}</h5>
                                                <p class="address"><span
                                                            class="lnr lnr-map"></span> {{$category_search_cus->location_work}}
                                                </p>
                                                <p class="address"><span
                                                            class="lnr lnr-database"></span> {{$category_search_cus->name_level_salary}}
                                                </p>
                                                <p class="address"><span class="lnr lnr-clock"></span> Time for
                                                    submission:
                                                    <b>{{$category_search_cus->time_for_submission}}</b></p>
                                                <p class="address"><span class="lnr lnr-shirt"></span> Number of
                                                    recruitment: <b>{{$category_search_cus->number_recruits }}</b> people</p>
                                                <p class="address"><span class="lnr lnr-user"></span> Gender
                                                    : {{$category_search_cus->gender }} </p>
                                            </div>
                                        </div>
                                    @endforeach
                                    <div class="row">
                                        <div class="col-sm-5"></div>
                                        <div class="col-sm-6">{{$category_search_custom->links()}}</div>
                                    ...
                                    @if(count($saved_jobs) > 0)
                                        @foreach($category_search_custom as $category_search_cus)
                                            ...
                                            @foreach($saved_jobs as $saved)
                                                ...
                                                @if($saved->id_post_news == $category_search_cus->id_posts and $saved->id_candidate == auth()->user()->id)
                                                    <?php
                                                    $flag = true;
                                                    ?>
                                                @endif
                                            ...
                                            @if($flag == true)
                                                <div class="single-post d-flex flex-row">
                                                    <div class="thumb">
                                                        <img src="{{ asset('img/post.png') }}" alt="">
                                                        <div hidden>{{$name = $category_search_cus->name_job_category}}</div>
                                                    ...
                                                    <div class="details">
                                                        ...
                                                        <p class="address"><span class="lnr lnr-shirt"></span>
                                                            Number of
                                                            recruitment: <b>{{$category_search_cus->number_recruits }}</b>
                                                            people
                                                        </p>
                                                        <p class="address"><span class="lnr lnr-user"></span> Gender
                                                            : {{$category_search_cus->gender }} </p>
                                                                    ...
                                                                    <form method="POST">
                                                                        <a>
                                                                            <button type="button"
                                                                                    ...
                                                                                    data-jobs-id-postnews="{{$category_search_cus->id_posts}}">
                                                                                <span class="lnr lnr-heart"></span>
                                                                            </button>
                                                                        </a>
                                                                    </form>
